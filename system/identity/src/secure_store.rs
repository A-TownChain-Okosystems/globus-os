...
fn zeroize_bytes(bytes: &mut [u8]) { for byte in bytes { *byte = 0; } }
...
mod tests {
    ...
    #[test] fn random_nonce_produces_distinct_ciphertexts() {
        ...
        first.replace_plaintext(b"same").unwrap(); second.replace_plaintext(b"same").unwrap();
        let (a, _) = first.into_parts(); let (b, _) = second.into_parts();
    ...
    #[test] fn wrong_key_is_rejected() {
        let mut store = store(); store.replace_plaintext(b"identity-record").unwrap();
        let (backend, _) = store.into_parts(); let wrong = TestKey(Zeroizing::new([8u8; 32]));
