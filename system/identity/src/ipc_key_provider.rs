...
pub trait IdentityKeyIpcTransport {
    fn load_key(&self, request: IdentityKeyLoadRequest) -> Result<Zeroizing<[u8; 32]>, SecureKeyServiceError>;
}

/// Capability-backed implementation of the identity key service over local IPC.
pub struct IpcIdentityKeyService<T> { transport: T, capability_token: u64 }
impl<T> IpcIdentityKeyService<T> {
    pub fn new(transport: T, capability_token: u64) -> Self { Self { transport, capability_token } }
}
impl<T: IdentityKeyIpcTransport> SecureKeyService for IpcIdentityKeyService<T> {
    fn load_identity_key(&self, key_id: SecureKeyId) -> Result<Zeroizing<[u8; 32]>, SecureKeyServiceError> {
        self.transport.load_key(IdentityKeyLoadRequest { key_handle: IdentityKeyHandle(key_id.0), capability_token: self.capability_token })
    }
}
