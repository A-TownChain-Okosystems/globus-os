//! Persistence boundary for the identity service.
//!
//! The identity service deliberately does not assume that a filesystem is secure enough
//! to hold credentials. Platform implementations must provide confidentiality, integrity,
//! atomic replacement, rollback protection, and secure deletion through the OS security
//! boundary. This module therefore defines the durable record contract separately from
//! the in-memory service state.
...
mod tests {
    ...
    fn record() -> IdentityRecord {
        ...
        let binding = IdentityBinding { user_id: user.clone(), wallet_address: address.clone(), chain_id: 600, network: "devnet".into(), key_version: 1 };
