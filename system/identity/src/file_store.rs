...
pub struct FileBlobStore { path: PathBuf }

impl FileBlobStore {
    pub fn open(path: impl Into<PathBuf>) -> Self { Self { path: path.into() } }
    pub fn path(&self) -> &Path { &self.path }

    fn read_state(&self) -> Result<(u8, u64, Vec<u8>), SecureStoreError> {
        let mut file = match File::open(&self.path) {
            Ok(file) => file,
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok((DELETED, 0, Vec::new())),
            Err(_) => return Err(SecureStoreError::BackendUnavailable),
        };
        let mut bytes = Vec::new();
        file.read_to_end(&mut bytes).map_err(|_| SecureStoreError::BackendUnavailable)?;
        if bytes.len() < HEADER_LEN || &bytes[..4] != MAGIC || bytes[4] != VERSION {
            return Err(SecureStoreError::InvalidEnvelope);
        }
        let state = bytes[5];
        if state != PRESENT && state != DELETED { return Err(SecureStoreError::InvalidEnvelope); }
        let generation = u64::from_le_bytes(bytes[6..14].try_into().map_err(|_| SecureStoreError::InvalidEnvelope)?);
        let len = u64::from_le_bytes(bytes[14..22].try_into().map_err(|_| SecureStoreError::InvalidEnvelope)?) as usize;
        if len != bytes.len().saturating_sub(HEADER_LEN) { return Err(SecureStoreError::InvalidEnvelope); }
        Ok((state, generation, bytes[HEADER_LEN..].to_vec()))
    }

    fn atomic_write(&self, state: u8, generation: u64, blob: &[u8]) -> Result<(), SecureStoreError> {
        if let Some(parent) = self.path.parent() { fs::create_dir_all(parent).map_err(|_| SecureStoreError::BackendUnavailable)?; }
        ...
        let result = (|| {
            ...
            file.write_all(&header).map_err(|_| SecureStoreError::BackendUnavailable)?;
            file.write_all(blob).map_err(|_| SecureStoreError::BackendUnavailable)?;
            file.sync_all().map_err(|_| SecureStoreError::BackendUnavailable)?;
...
impl BlobStore for FileBlobStore {
    ...
    fn replace(&mut self, blob: &[u8], generation: u64) -> Result<(), SecureStoreError> {
        ...
        if generation <= current && current != 0 {
            return Err(SecureStoreError::RollbackDetected { stored: current, incoming: generation });
        }
        self.atomic_write(PRESENT, generation, blob)
    }

    fn generation(&self) -> Result<u64, SecureStoreError> { Ok(self.read_state()?.1) }
...
mod tests {
    ...
    fn stale_generation_is_rejected_after_reopen() {
        ...
        assert_eq!(store.replace(b"old", 2).unwrap_err(), SecureStoreError::RollbackDetected { stored: 3, incoming: 2 });
        let _ = fs::remove_file(path);
    }
}
