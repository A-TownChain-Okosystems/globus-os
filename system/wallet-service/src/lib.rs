...
impl From<KeystoreError> for WalletServiceError {
    fn from(value: KeystoreError) -> Self { Self::Keystore(value) }
}
...
mod tests {
    ...
    impl KeyStore for TestStore {
        fn provision(&mut self, key_id: KeyId, _secret: Zeroizing<Vec<u8>>, algorithm: String) -> Result<ProtectedKey, KeystoreError> {
            Ok(ProtectedKey { metadata: KeyMetadata { key_id, algorithm, version: 1, hardware_backed: false } })
        }
        fn sign(&self, request: SignRequest) -> Result<Signature, KeystoreError> {
            Ok(Signature { algorithm: "test".into(), bytes: request.message })
        }
