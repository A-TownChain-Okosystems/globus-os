...
pub fn present(event: &CrashEvent) -> SystemErrorView {
    let (title, message) = match event.code {
        ErrorCode::DriverCrashed => (
            "Driver crashed",
            format!("The driver '{}' crashed and was isolated to protect the system.", event.driver),
        ),
        ErrorCode::DriverIsolated => (
            "Driver isolated",
            format!("The driver '{}' was isolated after a fault; the rest of the system remains running.", event.driver),
        ),
        ErrorCode::DriverRecoveryFailed => (
            "Driver recovery failed",
            format!("The driver '{}' could not be recovered and remains offline.", event.driver),
        ),
    };
    SystemErrorView {
