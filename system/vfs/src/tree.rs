...
pub enum FsError { InvalidPath, NotFound, AlreadyExists, NotDirectory, NotEmpty, PermissionDenied, Overflow }

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DirectoryEntry { pub name: String, pub inode: InodeId, pub file_type: FileType }

#[derive(Debug, Clone)]
struct Node { inode: Inode, data: Vec<u8>, children: BTreeMap<String, InodeId> }

#[derive(Debug, Clone)]
pub struct InodeTree { next_inode: u64, nodes: BTreeMap<InodeId, Node> }

impl Default for InodeTree { fn default() -> Self { Self::new() } }

impl InodeTree {
    pub fn new() -> Self {
        let root = Inode { id: InodeId(1), file_type: FileType::Directory, mode: FileMode { read: true, write: true, execute: true }, size: 0 };
        let mut nodes = BTreeMap::new();
        nodes.insert(InodeId(1), Node { inode: root, data: Vec::new(), children: BTreeMap::new() });
        Self { next_inode: 2, nodes }
    }

    pub fn root(&self) -> InodeId { InodeId(1) }
    ...
    fn parent_and_name(&self, path: &str) -> Result<(InodeId, String), FsError> {
        let normalized = crate::normalize(path).map_err(|_| FsError::InvalidPath)?;
        if normalized == "/" { return Err(FsError::InvalidPath); }
    ...
    pub fn stat(&self, path: &str) -> Result<Inode, FsError> {
        Ok(self.nodes.get(&self.lookup(path)?).ok_or(FsError::NotFound)?.inode.clone())
    }

    pub fn create(&mut self, path: &str, file_type: FileType, mode: FileMode) -> Result<InodeId, FsError> {
        let (parent, name) = self.parent_and_name(path)?;
        let parent_node = self.nodes.get(&parent).ok_or(FsError::NotFound)?;
        if parent_node.inode.file_type != FileType::Directory { return Err(FsError::NotDirectory); }
        if parent_node.children.contains_key(&name) { return Err(FsError::AlreadyExists); }
    ...
    pub fn list(&self, path: &str) -> Result<Vec<DirectoryEntry>, FsError> {
        ...
        if node.inode.file_type != FileType::Directory { return Err(FsError::NotDirectory); }
        Ok(node.children.iter().filter_map(|(name, child)| self.nodes.get(child).map(|n| DirectoryEntry { name: name.clone(), inode: *child, file_type: n.inode.file_type })).collect())
    }
    ...
    pub fn read(&self, id: InodeId, offset: u64, out: &mut [u8]) -> Result<usize, FileError> {
        ...
        if node.inode.file_type != FileType::Regular { return Err(FileError::NotFile); }
        let start = usize::try_from(offset).map_err(|_| FileError::Overflow)?;
        if start >= node.data.len() { return Ok(0); }
...
mod tests {
    ...
    fn creates_lists_and_round_trips() {
        ...
        fs.create("/home", FileType::Directory, FileMode::READ_WRITE).unwrap();
        let id = fs.create("/home/test", FileType::Regular, FileMode::READ_WRITE).unwrap();
