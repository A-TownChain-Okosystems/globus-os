...
pub fn parse_header(sector: &[u8]) -> Result<GptHeader, GptError> {
    ...
    let size=read_u32(sector, 12); if !(92..=512).contains(&size) || size as usize > sector.len() { return Err(GptError::BadHeaderSize); }
    let stored=read_u32(sector, 16); let mut h=sector[..size as usize].to_vec(); h[16..20].fill(0); if crc32(&h)!=stored { return Err(GptError::BadCrc); }
    let first=read_u64(sector, 48); let last=read_u64(sector, 56); let entries=read_u32(sector, 80); let entry_size=read_u32(sector, 84);
    if first>last || entries==0 || !(128..=4096).contains(&entry_size) || !entry_size.is_multiple_of(8) { return Err(GptError::InvalidRange); }
    Ok(GptHeader{first_usable_lba:first,last_usable_lba:last,partition_entry_lba:read_u64(sector, 72),partition_count:entries,partition_size:entry_size})
}
