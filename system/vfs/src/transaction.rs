...
impl Transaction {
    ...
    pub fn write(&mut self, block: u64, checksum: u32) -> Result<(), TransactionError> {
        if !self.active { return Err(TransactionError::Inactive); }
        if self.writes.len() >= 64 { return Err(TransactionError::TooManyWrites); }
        if self.writes.iter().any(|w| w.block == block) { return Ok(()); }
    ...
    pub fn commit(mut self, journal: &mut Journal) -> Result<u64, TransactionError> {
        ...
        for write in &self.writes {
            journal.append(self.sequence, write.block, JournalOp::Write { checksum: write.checksum })?;
        }
        self.active = false;
        Ok(self.sequence)
    }
...
mod tests {
    ...
    fn commit_records_write_intent() {
        ...
        let mut journal = Journal::new(4).unwrap();
        assert_eq!(tx.commit(&mut journal).unwrap(), 7);
    ...
    fn duplicate_blocks_are_coalesced() {
        ...
        tx.write(5, 1).unwrap();
        tx.write(5, 2).unwrap();
        assert_eq!(tx.writes(), &[PendingWrite { block: 5, checksum: 1 }]);
    }
}
