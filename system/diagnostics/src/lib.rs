...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Component { Kernel, Driver, Device, Service }
...
impl EventLog {
    ...
    pub fn record_driver_crash(&mut self, driver: &str, device_id: u64, timestamp_ns: u64) -> u64 {
        ...
        self.events.push(CrashEvent::driver_crash(sequence, driver, device_id, timestamp_ns));
        sequence
    }

    pub fn mark_recovered(&mut self, sequence: u64) -> bool {
        let Some(event) = self.events.iter_mut().find(|e| e.sequence == sequence) else { return false; };
    ...
    pub fn events(&self) -> &[CrashEvent] { &self.events }
    pub fn latest(&self) -> Option<&CrashEvent> { self.events.last() }
}
