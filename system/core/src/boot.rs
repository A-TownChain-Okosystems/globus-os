...
pub enum BootService { Security, Memory, Process, Ipc, Devices, Vfs, Identity, Wallet, Network, Graphics, Audio, Package, Update, Runtime, Settings }
...
pub struct BootStep { pub service: BootService, pub requires: &'static [BootService] }
...
const DEVICES: &[BootService] = &[BootService::Security, BootService::Memory, BootService::Process, BootService::Ipc, BootService::Devices];
const VFS: &[BootService] = &[BootService::Security, BootService::Memory, BootService::Process, BootService::Ipc, BootService::Devices];
...
pub const BOOT_PLAN: &[BootStep] = &[
    ...
    BootStep { service: BootService::Identity, requires: IDENTITY },
...
pub fn validate_boot_plan() -> bool {
    for (index, step) in BOOT_PLAN.iter().enumerate() {
        for requirement in step.requires {
            if !BOOT_PLAN[..index].iter().any(|candidate| candidate.service == *requirement) { return false; }
        }
    }
    true
}
...
mod tests {
    use super::*;
    #[test] fn canonical_plan_is_ordered() { assert!(validate_boot_plan()); }
    #[test] fn security_is_first() { assert_eq!(BOOT_PLAN.first().map(|s| s.service), Some(BootService::Security)); }
}
