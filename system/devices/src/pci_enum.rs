...
impl<'a> PciEnumerator<'a> {
        ...
        F: FnMut(PciFunction),
    {
        for bus in self.bus_start..=self.bus_end {
            for device in 0..32u8 {
                ...
                if self.ecam.read16(addr0, 0x00) == 0xffff {
                    continue;
                }
                ...
                for function in 0..functions {
                    ...
                    if self.ecam.read16(address, 0x00) == 0xffff {
                        continue;
                    }
    ...
    fn read_function(&self, address: PciAddress) -> Result<PciFunction, PciEnumError> {
        ...
        while i < 6 {
            ...
            if matches!(bars[i].map(|b| b.kind), Some(BarKind::Memory64)) {
                i += 1;
            }
            i += 1;
        }
    ...
    fn decode_bar(&self, address: PciAddress, index: u8) -> Result<Option<PciBar>, PciEnumError> {
        ...
        base.checked_add(size - 1)
            .ok_or(PciEnumError::AddressOverflow)?;
        Ok(Some(PciBar {
            index,
            kind,
            base,
