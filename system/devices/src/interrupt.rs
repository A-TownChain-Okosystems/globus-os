...
impl InterruptVector {
    pub fn valid(self) -> bool { (32..=255).contains(&self.0) }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum InterruptError { InvalidVector, InvalidCpu, AlreadyRouted, NotRouted }

#[derive(Debug, Default)]
pub struct InterruptController { routes: Vec<InterruptRoute> }

impl InterruptController {
    pub fn new() -> Self { Self::default() }

    pub fn route(&mut self, route: InterruptRoute) -> Result<(), InterruptError> {
        if !route.vector.valid() { return Err(InterruptError::InvalidVector); }
        if route.destination_cpu == u32::MAX { return Err(InterruptError::InvalidCpu); }
        if self.routes.iter().any(|r| r.vector == route.vector) { return Err(InterruptError::AlreadyRouted); }
    ...
    pub fn mask(&mut self, vector: InterruptVector, masked: bool) -> Result<(), InterruptError> {
        let route = self.routes.iter_mut().find(|r| r.vector == vector).ok_or(InterruptError::NotRouted)?;
        route.masked = masked;
        Ok(())
    }
    ...
    pub fn routes(&self) -> &[InterruptRoute] { &self.routes }
}
...
mod tests {
    ...
    fn routes_are_deterministic_and_maskable() {
        ...
        c.route(InterruptRoute { vector: InterruptVector(40), destination_cpu: 1, trigger: TriggerMode::Level, polarity: Polarity::ActiveHigh, masked: false }).unwrap();
