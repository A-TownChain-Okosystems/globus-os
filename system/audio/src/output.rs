...
impl RingAudioOutput {
    pub fn new(capacity_frames: usize) -> Result<Self, OutputError> {
        if capacity_frames == 0 { return Err(OutputError::InvalidCapacity); }
        Ok(Self { capacity_frames, spec: None, running: false, queued: 0 })
    }
...
impl AudioOutput for RingAudioOutput {
    fn open(&mut self, spec: AudioSpec) -> Result<(), OutputError> {
        ...
        if self.spec.is_some() { return Err(OutputError::AlreadyOpen); }
        self.spec = Some(spec);
        self.queued = 0;
        Ok(())
    }
    ...
    fn start(&mut self) -> Result<(), OutputError> {
        if self.spec.is_none() { return Err(OutputError::NotOpen); }
        self.running = true;
        Ok(())
    }
    ...
    fn write(&mut self, frame: &PcmFrame) -> Result<usize, OutputError> {
        ...
        if frame.spec != spec { return Err(OutputError::SpecMismatch); }
        let available = self.capacity_frames.saturating_sub(self.queued);
        if frame.frames() > available { return Err(OutputError::BufferFull); }
        self.queued += frame.frames();
        Ok(frame.frames())
    }

    fn queued_frames(&self) -> usize { self.queued }
    fn latency_frames(&self) -> usize { self.queued }
}

impl Default for RingAudioOutput {
    fn default() -> Self { Self::new(48_000).expect("non-zero default capacity") }
}
