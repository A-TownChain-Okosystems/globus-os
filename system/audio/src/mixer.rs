...
use crate::codec::{AudioSpec, PcmFrame, CodecError};
...
impl Default for PcmMixer {
    fn default() -> Self { Self { gain: 1.0, muted: false } }
}
...
mod tests {
    ...
    fn spec() -> AudioSpec { AudioSpec { sample_rate: 48_000, channels: 2, format: AudioFormat::PcmF32Le } }

    #[test]
    fn mixes_and_clips() {
        let frame = PcmFrame::new(spec(), vec![0.8, -0.8]).unwrap();
        let mixed = PcmMixer::new().mix(spec(), &[frame.clone(), frame]).unwrap();
