...
use crate::{AudioDecoder, AudioOutput, MediaPlayer, MixerError, PcmMixer, OutputError, CodecError};
...
impl From<CodecError> for PipelineError { fn from(e: CodecError) -> Self { Self::Decoder(e) } }
impl From<MixerError> for PipelineError { fn from(e: MixerError) -> Self { Self::Mixer(e) } }
impl From<OutputError> for PipelineError { fn from(e: OutputError) -> Self { Self::Output(e) } }
...
impl<D: AudioDecoder, O: AudioOutput> MediaPipeline<D, O> {
    pub fn new(player: MediaPlayer, decoder: D, output: O) -> Self {
        Self { player, decoder, mixer: PcmMixer::new(), output }
    }

    pub fn mixer(&self) -> &PcmMixer { &self.mixer }
    pub fn mixer_mut(&mut self) -> &mut PcmMixer { &mut self.mixer }
    pub fn output(&self) -> &O { &self.output }
    pub fn output_mut(&mut self) -> &mut O { &mut self.output }
    ...
    pub fn pump(&mut self, max_frames: usize) -> Result<usize, PipelineError> {
        if self.player.queue().current().is_none() { return Err(PipelineError::NoTrack); }
