...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum WindowLifecycle { Created, Attached, Suspended, Detached, Closed }
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
pub trait AudioControl { fn set_volume(&mut self, percent: u8); fn set_muted(&mut self, muted: bool); }

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct MediaWindow {
    pub id: u64, pub surface_id: u64, pub mode: MediaWindowMode, pub playback: PlaybackState,
    pub lifecycle: WindowLifecycle, pub position_us: u64, pub duration_us: Option<u64>,
    pub seek_step_us: u64, pub volume_percent: u8, pub muted: bool,
}

impl MediaWindow {
    pub fn new(id: u64, surface_id: u64) -> Self {
        Self { id, surface_id, mode: MediaWindowMode::Windowed, playback: PlaybackState::Stopped,
            lifecycle: WindowLifecycle::Created, position_us: 0, duration_us: None,
            seek_step_us: 10_000_000, volume_percent: 100, muted: false }
    }
    pub fn attach(&mut self) { if self.lifecycle != WindowLifecycle::Closed { self.lifecycle = WindowLifecycle::Attached; } }
    pub fn suspend(&mut self) { if self.lifecycle == WindowLifecycle::Attached { self.lifecycle = WindowLifecycle::Suspended; } }
    pub fn resume(&mut self) { if self.lifecycle == WindowLifecycle::Suspended { self.lifecycle = WindowLifecycle::Attached; } }
    pub fn detach(&mut self) { if self.lifecycle != WindowLifecycle::Closed { self.lifecycle = WindowLifecycle::Detached; } }
    pub fn close(&mut self) { self.lifecycle = WindowLifecycle::Closed; self.playback = PlaybackState::Stopped; }
    ...
    pub fn play(&mut self) { if self.lifecycle != WindowLifecycle::Closed { self.playback = PlaybackState::Playing; } }
    pub fn pause(&mut self) { if self.playback == PlaybackState::Playing { self.playback = PlaybackState::Paused; } }
    ...
    pub fn apply(&mut self, input: MediaInput) {
        match input {
            ...
            MediaInput::Stop => self.stop(), MediaInput::SeekForward => self.seek_by(self.seek_step_us as i64),
    ...
    pub fn sync_audio<A: AudioControl>(&self, audio: &mut A) { audio.set_volume(self.volume_percent); audio.set_muted(self.muted); }
}
