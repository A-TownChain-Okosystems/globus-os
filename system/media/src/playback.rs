...
use crate::{AvSyncPolicy, FrameQueue, MediaError, SyncAction, TimedVideoFrame, VideoCodecDecoder, VideoFrame};

#[derive(Debug, Default, Clone, Copy, PartialEq, Eq)]
pub struct PlaybackStats { pub submitted: u64, pub decoded: u64, pub presented: u64, pub dropped: u64, pub held: u64, pub resyncs: u64 }
...
impl<D: VideoCodecDecoder> CodecVideoSession<D> {
    pub fn new(decoder: D, queue_capacity: usize, sync: AvSyncPolicy) -> Self {
        Self { decoder, queue: FrameQueue::new(queue_capacity), sync, stats: PlaybackStats::default(), next_audio_pts_us: 0 }
    }

    pub fn stats(&self) -> PlaybackStats { self.stats }
    pub fn queue_len(&self) -> usize { self.queue.len() }
    pub fn set_audio_clock(&mut self, pts_us: u64) { self.next_audio_pts_us = pts_us; }

    pub fn submit_packet(&mut self, packet: &[u8], pts_us: u64, dts_us: u64, keyframe: bool) -> Result<(), MediaError> {
    ...
    pub fn next_for_presentation(&mut self) -> Option<(u64, VideoFrame)> {
        ...
        match self.sync.action(self.next_audio_pts_us, pts) {
            ...
            SyncAction::Hold => { self.stats.held = self.stats.held.saturating_add(1); None }
            SyncAction::Drop => {
