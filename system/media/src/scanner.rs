...
pub struct FileRecord { pub uri: String, pub size_bytes: u64, pub modified_unix_ms: u64 }

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ScanEvent { pub change: ScanChange, pub record: FileRecord }

pub trait MediaFileSource { fn list_files(&mut self, root: &str) -> Result<Vec<FileRecord>, String>; }
pub trait MediaWatchSource { fn poll(&mut self) -> Result<Vec<ScanEvent>, String>; }
...
impl MediaScanner {
    ...
    fn stable_id(uri: &str) -> u64 {
        ...
        for byte in uri.as_bytes() { hash ^= *byte as u64; hash = hash.wrapping_mul(0x100000001b3); }
        hash
    }
    ...
    pub fn metadata_for(&mut self, file: &FileRecord) -> Option<MediaMetadata> {
        ...
        Some(MediaMetadata {
            id: Self::stable_id(&file.uri), uri: file.uri.clone(), title, mime,
            size_bytes: file.size_bytes, modified_unix_ms: file.modified_unix_ms,
            content_hash: None, duration_ms: None, width: None, height: None,
        })
    }
