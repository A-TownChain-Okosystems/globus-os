...
impl Default for AvSyncPolicy {
    fn default() -> Self {
        Self { hold_threshold_us: 20_000, drop_threshold_us: 80_000, resync_threshold_us: 250_000 }
    }
}
...
impl AvSyncPolicy {
    pub fn action(&self, audio_pts_us: u64, video_pts_us: u64) -> SyncAction {
        ...
        if magnitude >= self.resync_threshold_us { SyncAction::Resync }
        else if delta > self.hold_threshold_us as i128 { SyncAction::Hold }
...
impl AvSyncPolicy {
    pub fn decide(&self, audio_pts_us: u64, video_pts_us: u64) -> SyncDecision {
        SyncDecision { audio_pts_us, video_pts_us, action: self.action(audio_pts_us, video_pts_us) }
    }
}
