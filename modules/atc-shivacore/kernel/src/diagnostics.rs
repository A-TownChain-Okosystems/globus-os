...
pub enum DriverState { Active, Faulted, Isolated }
...
impl Default for DriverFaultManager { fn default() -> Self { Self::new() } }

impl DriverFaultManager {
    pub const fn new() -> Self { Self { events: EventLog::new() } }

    pub fn report_crash(&mut self, driver: &str, device_id: u64, timestamp_ns: u64) -> DriverFault {
        let sequence = self.events.record_driver_crash(driver, device_id, timestamp_ns);
        DriverFault { driver: String::from(driver), device_id, state: DriverState::Faulted, event_sequence: sequence }
    }

    /// Isolate a faulted driver without bringing down the kernel.
    pub fn isolate(&mut self, fault: &mut DriverFault) -> bool {
        if fault.state != DriverState::Faulted { return false; }
        if !self.events.mark_recovered(fault.event_sequence) { return false; }
        fault.state = DriverState::Isolated;
        true
    }

    pub fn events(&self) -> &[CrashEvent] { self.events.events() }
    pub fn latest(&self) -> Option<&CrashEvent> { self.events.latest() }
}
