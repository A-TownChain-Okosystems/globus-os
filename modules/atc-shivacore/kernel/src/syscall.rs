...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum DispatchError { Abi(AbiError) }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct SyscallDispatcher { monotonic_ticks: u64 }

impl SyscallDispatcher {
    pub const fn new() -> Self { Self { monotonic_ticks: 0 } }

    pub fn dispatch(&mut self, pid: Pid, request: SyscallRequest, capabilities: &CapabilityTable) -> SyscallResponse {
    ...
    fn check_capability(&self, pid: Pid, handle: Option<CapabilityHandle>, capabilities: &CapabilityTable, resource_type: ResourceType, resource_id: u64, rights: Rights) -> bool {
        let Some(handle) = handle else { return false };
        capabilities.get(CapId(handle.0)).map(|cap| {
            cap.owner == pid && cap.resource_type == resource_type && cap.resource_id == resource_id && cap.rights.has(rights)
        }).unwrap_or(false)
    }
}
...
mod tests {
    ...
    fn request(syscall: Syscall) -> SyscallRequest {
        SyscallRequest { abi_version: libshivacore::ABI_VERSION, syscall_id: syscall.id(), capability: None, arg0: 0, arg1: 0, payload_len: 0 }
    }
