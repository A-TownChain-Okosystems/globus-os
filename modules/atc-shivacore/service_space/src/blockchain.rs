...
pub struct ProposalPipeline{mempool:Arc<shivacore::mempool::MemoryPool>,validator:Arc<shivacore::mempool::TxValidator>,consensus:Arc<ConsensusEngine>,chain:Arc<BlockChain>,state:Arc<shivacore::mempool::StateDb>,our_did:String,block_height:Mutex<u64>}
impl ProposalPipeline{pub fn new(mempool:Arc<shivacore::mempool::MemoryPool>,validator:Arc<shivacore::mempool::TxValidator>,consensus:Arc<ConsensusEngine>,chain:Arc<BlockChain>,state:Arc<shivacore::mempool::StateDb>,our_did:String)->Self{ProposalPipeline{mempool,validator,consensus,chain,state,our_did,block_height:Mutex::new(0)}}
...
pub fn cleanup_mempool(&self,now:u64)->usize{self.mempool.cleanup(now)} pub fn chain(&self)->&Arc<BlockChain>{&self.chain} pub fn mempool(&self)->&Arc<shivacore::mempool::MemoryPool>{&self.mempool} pub fn consensus(&self)->&Arc<ConsensusEngine>{&self.consensus} pub fn state(&self)->&Arc<shivacore::mempool::StateDb>{&self.state} pub fn current_height(&self)->u64{*self.block_height.lock()}}
...
mod tests {
    use super::*;
    use shivacore::mempool::{NonceTracker, MemoryPool, TxType};
    use crate::consensus::ConsensusEngine;
    fn setup() -> ProposalPipeline {
        ...
        let val = Arc::new(shivacore::mempool::TxValidator::new(state.clone(), nonces, 1));
